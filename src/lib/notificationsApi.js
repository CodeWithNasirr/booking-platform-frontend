...
const PLATFORM_BASE = "/api/v1/platform/notifications/feed";
...
export async function fetchPlatformNotificationSummary() {
  return platformFetch(`${PLATFORM_BASE}/summary/`, { method: "GET" });
}

export async function fetchPlatformNotifications({ limit = 20, category, unread } = {}) {
  const qs = new URLSearchParams();
  if (limit) qs.set("limit", String(limit));
  if (category) qs.set("category", category);
  if (unread) qs.set("unread", "1");
  const suffix = qs.toString() ? `?${qs.toString()}` : "";
  return platformFetch(`${PLATFORM_BASE}/${suffix}`, { method: "GET" });
}

export async function markPlatformNotificationRead(id) {
  return platformFetch(`${PLATFORM_BASE}/${id}/read/`, { method: "POST" });
}

export async function markAllPlatformNotificationsRead(category) {
  const qs = category ? `?category=${encodeURIComponent(category)}` : "";
  return platformFetch(`${PLATFORM_BASE}/read-all/${qs}`, { method: "POST" });
}
