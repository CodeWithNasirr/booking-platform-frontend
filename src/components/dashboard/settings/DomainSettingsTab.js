...
const FAQ = [
  { qKey: 'settings.domain.faq.whatIs', aKey: 'settings.domain.faq.whatIsAnswer' },
  { qKey: 'settings.domain.faq.cost', aKey: 'settings.domain.faq.costAnswer' },
  { qKey: 'settings.domain.faq.howLong', aKey: 'settings.domain.faq.howLongAnswer' },
  { qKey: 'settings.domain.faq.canChange', aKey: 'settings.domain.faq.canChangeAnswer' },
]

const STATUS_CONFIG = {
  pending:  { labelKey: 'settings.domain.status.pending',  color: 'text-amber-700 bg-amber-50',  icon: Clock },
  verified: { labelKey: 'settings.domain.status.verified', color: 'text-green-700 bg-green-50',  icon: CheckCircle2 },
  failed:   { labelKey: 'settings.domain.status.failed',   color: 'text-red-700 bg-red-50',      icon: XCircle },
  expired:  { labelKey: 'settings.domain.status.expired',  color: 'text-gray-700 bg-gray-50',    icon: Clock },
}
...
export default function DomainSettingsTab({
  ...
  const handleVerify = async (domainId) => {
    ...
    try {
      await verifyDomain(activeTenant, domainId)
      onDomainsChange?.()
    } catch (err) {
      if (err.status === 422 && err.data) {
        setVerifyResult({ domainId, ...err.data })
      } else if (err.status === 429) {
        alert(err.data?.detail || t('settings.domain.rateLimited'))
      } else {
        alert(err.message)
      }
      onDomainsChange?.()
  ...
  const handleRemove = async (domainId) => {
    if (!confirm(t('settings.domain.confirmRemove'))) return
  ...
  return (
    <div className="space-y-8">
      <Section icon={Edit2} iconBg="bg-amber-100" iconColor="text-amber-600" title={t('settings.domain.websiteLink')}>
        {editingSlug ? (
          <div className="flex items-center gap-3">
            ...
            <button onClick={handleSaveSlug} disabled={slugSaving}
              ...
              {slugSaving ? <Loader2 className="w-4 h-4 animate-spin" /> : t('common.save')}
            </button>
            <button onClick={() => { setEditingSlug(false); setSlugValue(tenantSlug) }}
              className="px-4 py-3 rounded-xl border border-gray-300 text-gray-600 text-sm hover:bg-gray-50">{t('common.cancel')}</button>
          </div>
        ) : (
          <div className="flex items-center gap-3">
            ...
            <a href={websiteUrl || '#'} target="_blank" rel="noopener noreferrer"
              ...
              {websiteUrl || subdomain?.domain || t('settings.domain.notConfigured')}
            </a>
      ...
      <Section icon={Globe} iconBg="bg-blue-100" iconColor="text-blue-600" title={t('settings.domain.domainManagement')}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <button onClick={() => setShowLinkModal(true)}
            ...
            <div>
              <p className="font-bold text-gray-900 mb-1">{t('settings.domain.linkExisting')}</p>
              <p className="text-sm text-gray-500">{t('settings.domain.linkExistingHint')}</p>
            </div>
          </button>
          <a href={websiteUrl || '#'} target="_blank" rel="noopener noreferrer"
            ...
            <div>
              <p className="font-bold text-gray-900 mb-1">{t('settings.domain.visitWebsite')}</p>
              <p className="text-sm text-gray-500">{t('settings.domain.visitWebsiteHint')}</p>
            </div>
          </a>
        </div>
      </Section>
      ...
      {verifyResult?.errors?.length > 0 && (
        <div className="rounded-xl border-2 border-red-200 bg-red-50/50 p-6 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-base font-bold text-red-900">{t('settings.domain.dnsIssues')}</h3>
          ...
          {verifyResult.checks && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <CheckBadge label={t('settings.domain.checks.txt')} ok={verifyResult.checks.txt_verified} />
              <CheckBadge label={t('settings.domain.checks.cname')} ok={verifyResult.checks.cname_correct} />
              <CheckBadge label={t('settings.domain.checks.noA')} ok={!verifyResult.checks.a_record_present} bad={verifyResult.checks.a_record_present} />
              <CheckBadge label={t('settings.domain.checks.noAAAA')} ok={!verifyResult.checks.aaaa_record_present} bad={verifyResult.checks.aaaa_record_present} />
            </div>
          )}
          ...
          {verifyResult.next_steps?.length > 0 && (
            <div className="bg-white rounded-xl p-4 border border-red-200">
              <p className="text-xs font-bold text-gray-900 uppercase tracking-wider mb-2">{t('settings.domain.stepsToFix')}</p>
      ...
      {domains?.length > 0 && (
        <Section icon={Shield} iconBg="bg-green-100" iconColor="text-green-600" title={t('settings.domain.connectedDomains')}
          badge={`${domains.length} ${domains.length !== 1 ? t('settings.domain.domainsPlural') : t('settings.domain.domainSingular')}`}>
          <div className="overflow-x-auto -mx-6">
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50/60">
                  <th className="text-left px-6 py-3 text-xs font-bold text-gray-500 uppercase tracking-wider">{t('settings.domain.table.domain')}</th>
                  <th className="text-left px-6 py-3 text-xs font-bold text-gray-500 uppercase tracking-wider">{t('settings.domain.table.type')}</th>
                  <th className="text-left px-6 py-3 text-xs font-bold text-gray-500 uppercase tracking-wider">{t('settings.domain.table.status')}</th>
                  <th className="text-left px-6 py-3 text-xs font-bold text-gray-500 uppercase tracking-wider">{t('settings.domain.table.routing')}</th>
                  <th className="text-right px-6 py-3 text-xs font-bold text-gray-500 uppercase tracking-wider">{t('settings.domain.table.actions')}</th>
                </tr>
              </thead>
                    ...
                    <tr key={d.id} className="hover:bg-gray-50/50">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          ...
                          {d.is_primary && (
                            <span className="px-2 py-0.5 text-[10px] font-bold rounded-full bg-amber-100 text-amber-700 border border-amber-200">{t('settings.domain.primary')}</span>
                          )}
                        </div>
                      ...
                      <td className="px-6 py-4">
                        <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${d.is_subdomain ? 'bg-blue-50 text-blue-700' : 'bg-purple-50 text-purple-700'}`}>
                          {d.is_subdomain ? t('settings.domain.subdomain') : t('settings.domain.custom')}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        {d.is_subdomain ? (
                          <span className="text-xs text-green-600 font-medium">{t('settings.domain.routingAuto')}</span>
                        ) : d.cname_verified ? (
                          <span className="flex items-center gap-1 text-xs text-green-600 font-medium">
                            <CheckCircle2 className="w-3 h-3" /> {t('settings.domain.routingOk')}
                          </span>
                        ) : (
                          <span className="flex items-center gap-1 text-xs text-amber-600 font-medium">
                            <AlertCircle className="w-3 h-3" /> {t('settings.domain.routingPending')}
                          </span>
                        )}
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-end gap-1.5">
                          {d.is_custom  && (
                            <>
                              <button onClick={() => handleCheckDns(d)} title={t('settings.domain.dnsStatus')}
                              ...
                              }} className="p-1.5 rounded-lg text-gray-400 hover:text-green-600 hover:bg-green-50" title={t('settings.domain.checkSSL')}>
                          ...
                          {d.is_verified && !d.is_primary && (
                            <button onClick={() => handleSetPrimary(d.id)}
                              ...
                              <Star className="w-3 h-3" /> {t('settings.domain.setPrimary')}
                            </button>
                          )}
                          {d.is_custom && (
      ...
      <Section icon={Palette} iconBg="bg-pink-100" iconColor="text-pink-600" title={t('settings.domain.brandColors')}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <ColorPicker label={t('settings.domain.primaryColor')} value={branding?.primary_color || '#8B1E3F'}
            onChange={(v) => onBrandingChange({ ...branding, primary_color: v })} />
          <ColorPicker label={t('settings.domain.secondaryColor')} value={branding?.secondary_color || '#10B981'}
        ...
        <div className="flex justify-end mt-6">
          <button onClick={onSaveBranding} disabled={saving}
            ...
            {t('settings.domain.saveColors')}
          </button>
        </div>
      </Section>

      <Section icon={HelpCircle} iconBg="bg-gray-100" iconColor="text-gray-600" title={t('settings.domain.commonQuestions')}>
          ...
          {FAQ.map((item, i) => (
            <div key={i}>
              <button onClick={() => setExpandedFaq(expandedFaq === i ? null : i)}
                ...
                <span className="text-sm font-medium text-gray-900">{t(item.qKey)}</span>
                <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform ml-4 flex-shrink-0 ${expandedFaq === i ? 'rotate-180' : ''}`} />
              </button>
              {expandedFaq === i && (
                <div className="px-6 pb-4"><p className="text-sm text-gray-600 leading-relaxed">{t(item.aKey)}</p></div>
              )}
            </div>
          ))}
        </div>
      ...
      {dnsModal && (
        <DnsStatusModal domain={dnsModal} onClose={() => setDnsModal(null)} t={t} />
      )}
    </div>
  )
}
...
function DnsStatusModal({ domain, onClose, t }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl max-h-[85vh] flex flex-col">
        <div className="px-6 py-5 border-b border-gray-200 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-bold text-gray-900">{t('settings.domain.dnsModal.title')}</h2>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          {domain.loading ? (
            <div className="flex items-center justify-center py-12">
              ...
              <span className="ml-3 text-sm text-gray-500">{t('settings.domain.dnsModal.checking')}</span>
            </div>
          ) : domain.error ? (
            <div className="p-4 rounded-xl bg-red-50 border border-red-200 text-sm text-red-700">{domain.error}</div>
          ) : dns ? (
            <>
              <StatusRow label={t('settings.domain.dnsModal.txt')} ok={dns.checks?.txt_verified ?? dns.txt_found}
                detail={dns.checks?.txt_verified || dns.txt_found ? t('settings.domain.dnsModal.txtFound') : t('settings.domain.dnsModal.txtNotFound')} />
              <StatusRow label={t('settings.domain.dnsModal.cname')} ok={dns.checks?.cname_correct ?? dns.cname_correct}
                detail={dns.details?.cname?.found
                  ? `${t('settings.domain.dnsModal.pointsTo')}: ${dns.details.cname.target || 'unknown'}`
                  : t('settings.domain.dnsModal.noCname')} />
              <StatusRow label={t('settings.domain.dnsModal.aRecord')}
                ok={!(dns.checks?.a_record_present ?? dns.details?.a_record?.found)}
                detail={dns.details?.a_record?.found
                  ? `${t('settings.domain.dnsModal.found')}: ${dns.details.a_record.addresses?.join(', ')} — ${t('settings.domain.dnsModal.remove')}`
                  : t('settings.domain.dnsModal.noneFound')} />
              <StatusRow label={t('settings.domain.dnsModal.aaaaRecord')}
                ok={!(dns.checks?.aaaa_record_present ?? dns.details?.aaaa_record?.found)}
                detail={dns.details?.aaaa_record?.found
                  ? `${t('settings.domain.dnsModal.found')}: ${dns.details.aaaa_record.addresses?.join(', ')} — ${t('settings.domain.dnsModal.remove')}`
                  : t('settings.domain.dnsModal.noneFound')} />
              ...
              <div className={`p-3 rounded-xl border text-center text-sm font-bold ${
                ...
                {dns.dns_ready ? t('settings.domain.dnsModal.ready') : t('settings.domain.dnsModal.notReady')}
              </div>
              ...
              {dns.next_steps?.length > 0 && (
                <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
                  <p className="text-xs font-bold text-gray-500 uppercase mb-2">{t('settings.domain.dnsModal.fixSteps')}</p>
              ...
              {result.expected_records?.length > 0 && (
                <div>
                  <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">{t('settings.domain.dnsModal.requiredRecords')}</p>
        ...
        <div className="px-6 py-4 border-t border-gray-200 flex justify-end">
          <button onClick={onClose} className="px-5 py-2.5 rounded-xl border border-gray-300 text-gray-700 font-medium hover:bg-gray-50 text-sm">{t('common.close')}</button>
        </div>
      </div>
    </div>
  )
      ...
      <div className="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl max-h-[90vh] flex flex-col">
        <div className="px-6 py-5 border-b border-gray-200 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-gray-900">{t('settings.domain.linkModal.title')}</h2>
              <p className="text-sm text-gray-500">{t('settings.domain.linkModal.subtitle')}</p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-gray-100 rounded-xl"><X className="w-5 h-5 text-gray-400" /></button>
        </div>
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          {!result ? (
            <>
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">{t('settings.domain.linkModal.domainName')}</label>
                <input type="text" value={domain}
                  onChange={(e) => setDomain(e.target.value.toLowerCase().replace(/[^a-z0-9.-]/g, ''))}
                  placeholder={t('settings.domain.linkModal.domainPlaceholder')} autoFocus
                ...
                <p className="text-xs text-gray-500 mt-1.5">{t('settings.domain.linkModal.domainHint')}</p>
              </div>
              <div className="p-4 rounded-xl bg-blue-50 border border-blue-200">
                <div className="flex gap-3">
                  <AlertCircle className="w-5 h-5 text-blue-600 flex-shrink-0 mt-0.5" />
                  <div>
                    <p className="text-sm font-bold text-blue-900 mb-1">{t('settings.domain.linkModal.beforeStart')}</p>
                    <p className="text-xs text-blue-700 leading-relaxed">
                      {t('settings.domain.linkModal.beforeStartDesc')}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <>
              <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-200">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <p className="text-sm font-bold text-emerald-900">{t('settings.domain.linkModal.domainAdded')}</p>
                </div>
                <p className="text-xs text-emerald-700">{t('settings.domain.linkModal.followSteps')}</p>
              </div>

              {result.important && (
                <div className="p-3 rounded-xl bg-amber-50 border border-amber-200">
                  <p className="text-xs text-amber-700"><strong>{t('common.important')}:</strong> {result.important}</p>
                </div>
              )}
              ...
              {result.is_root_domain && result.recommended_domain && (
                <div className="p-3 rounded-xl bg-blue-50 border border-blue-200">
                  <p className="text-xs text-blue-700">
                    <strong>{t('common.recommended')}:</strong> {t('settings.domain.linkModal.use')} <code className="font-mono bg-blue-100 px-1 rounded">{result.recommended_domain}</code> {t('settings.domain.linkModal.forBest')}
                  </p>
                </div>
              )}
              ...
              {result.setup_steps?.length > 0 && (
                <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
                  <p className="text-xs font-bold text-gray-900 uppercase tracking-wider mb-3">{t('settings.domain.linkModal.setupSteps')}</p>
              ...
              <div className="space-y-3">
                <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">{t('settings.domain.linkModal.dnsRecords')}</p>
                {result.dns_records?.map((record, i) => (
                  <div key={i} className="p-4 rounded-xl bg-white border border-gray-200 shadow-sm">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-red-100 text-red-700">{t('common.required')}</span>
                    </div>
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-gray-500 w-12">{t('settings.domain.linkModal.host')}:</span>
                        <code className="text-xs font-mono bg-gray-50 px-2 py-1 rounded border flex-1">{record.host}</code>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-gray-500 w-12">{t('settings.domain.linkModal.value')}:</span>
        ...
        <div className="px-6 py-4 border-t border-gray-200 flex justify-end gap-3">
          {!result ? (
            <>
              <button onClick={onClose} className="px-5 py-2.5 rounded-xl border border-gray-300 text-gray-700 font-medium hover:bg-gray-50 text-sm">{t('common.cancel')}</button>
              <button onClick={handleLink} disabled={loading || !domain.trim()}
                ...
                {loading && <Loader2 className="w-4 h-4 animate-spin" />} {t('settings.domain.linkModal.linkDomain')}
              </button>
            </>
          ) : (
            <button onClick={onSuccess} className="px-5 py-2.5 rounded-xl text-white bg-gradient-to-br from-[#8B1E3F] to-[#6B1630] hover:opacity-90 shadow-md font-medium text-sm">{t('common.done')}</button>
          )}
        </div>
      </div>
    </div>
