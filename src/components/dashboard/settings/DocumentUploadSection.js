...
const STATUS_CONFIGS = {
  pending:  { icon: Clock, color: "text-amber-600", bg: "bg-amber-50", border: "border-amber-200", labelKey: "settings.documents.status.pending" },
  approved: { icon: CheckCircle, color: "text-emerald-600", bg: "bg-emerald-50", border: "border-emerald-200", labelKey: "settings.documents.status.approved" },
  rejected: { icon: XCircle, color: "text-red-600", bg: "bg-red-50", border: "border-red-200", labelKey: "settings.documents.status.rejected" },
  expired:  { icon: AlertCircle, color: "text-gray-500", bg: "bg-gray-100", border: "border-gray-200", labelKey: "settings.documents.status.expired" },
};
...
export default function DocumentUploadSection({ activeTenant }) {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      setError(t('settings.documents.loadFailed'));
    } finally {
      setLoading(false);
    }
  }, [activeTenant, t]);
  ...
  const handleDelete = async (doc) => {
    if (!confirm(t('settings.documents.confirmDelete', { type: doc.document_type_display }))) return;
    try {
      ...
      if (res.ok) {
        ...
        setError(data.detail || t('settings.documents.deleteFailed'));
      }
    } catch {
      setError(t('settings.documents.deleteFailed'));
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h3 className="text-base font-bold text-gray-900">{t('settings.documents.title')}</h3>
      ...
      {docTypes.length > 0 && (
        <div className="p-4 bg-blue-50 border border-blue-200 rounded-xl">
          <p className="text-sm font-semibold text-blue-900 mb-2">{t('settings.documents.recommended')}</p>
      ...
      {documents.length === 0 ? (
        <div className="text-center py-10 bg-gray-50 rounded-xl border-2 border-dashed border-gray-300">
          ...
          <p className="text-sm font-medium text-gray-700">{t('settings.documents.noDocuments')}</p>
          <p className="text-xs text-gray-500 mt-1">{t('settings.documents.noDocumentsHint')}</p>
        </div>
      ) : (
        <div className="space-y-2">
          {documents.map(doc => (
            <DocumentRow key={doc.id} doc={doc} onDelete={handleDelete} t={t} />
          ))}
        </div>
      )}
...
function VerificationStatusBanner({ verification, t }) {
  ...
  if (admin_override) {
    return (
      <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-200 flex items-start gap-3">
        ...
        <div>
          <p className="text-sm font-semibold text-emerald-900">{t('settings.documents.verification.override')}</p>
          <p className="text-xs text-emerald-700 mt-0.5">{t('settings.documents.verification.overrideDesc')}</p>
        </div>
      </div>
    );
  }
  ...
  if (level === "verified") {
    return (
      <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-200 flex items-start gap-3">
        ...
        <div>
          <p className="text-sm font-semibold text-emerald-900">{t('settings.documents.verification.verified')}</p>
          <p className="text-xs text-emerald-700 mt-0.5">{t('settings.documents.verification.verifiedDesc')}</p>
        </div>
      </div>
    );
  }
      ...
      <div className="p-4 rounded-xl bg-amber-50 border border-amber-200 flex items-start gap-3">
        ...
        <div>
          <p className="text-sm font-semibold text-amber-900">
            {t('settings.documents.verification.gracePeriod', { days: days_remaining_in_grace })}
          </p>
          <p className="text-xs text-amber-700 mt-0.5">
            {t('settings.documents.verification.uploadRequired')}
          </p>
  ...
  return (
    <div className="p-4 rounded-xl bg-red-50 border border-red-200 flex items-start gap-3">
      ...
      <div>
        <p className="text-sm font-semibold text-red-900">{t('settings.documents.verification.required')}</p>
        <p className="text-xs text-red-700 mt-0.5">
          {t('settings.documents.verification.requiredDesc', { restricted: !can_accept_payments })}
        </p>
      </div>
    </div>
  );
...
function DocumentRow({ doc, onDelete, t }) {
          ...
          <div className="flex-1 min-w-0">
            ...
            {doc.document_number && (
              <p className="text-xs text-gray-600 mt-1">
                <span className="text-gray-400">{t('settings.documents.number')}</span> <code className="font-mono">{doc.document_number}</code>
              </p>
            )}
            {doc.expiry_date && (
              <p className="text-xs text-gray-500 mt-1 flex items-center gap-1">
                <Calendar className="w-3 h-3" />
                {t('settings.documents.expires')}: {new Date(doc.expiry_date).toLocaleDateString()}
              </p>
            )}
            {doc.status === "rejected" && doc.rejection_reason && (
              <div className="mt-2 p-2 bg-red-100 rounded text-xs text-red-800">
                <strong>{t('settings.documents.rejected')}:</strong> {doc.rejection_reason}
              </div>
            )}
          </div>
        </div>
...
function UploadModal({ activeTenant, docTypes, existingDocs, onClose, onUploaded, t }) {
  ...
  const selectFile = (f) => {
    ...
    if (!allowedExts.includes(ext)) {
      setError(t('settings.documents.upload.fileTypeNotAllowed', { ext }));
      return;
    }
    if (f.size > 10 * 1024 * 1024) {
      setError(t('settings.documents.upload.fileTooLarge'));
      return;
    }
    setFile(f);
    setError(null);
  ...
  const handleSubmit = async () => {
    if (!file) {
      setError(t('settings.documents.upload.selectFile'));
      return;
    }
    setUploading(true);
    setProgress(0);
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        ...
        xhr.addEventListener("load", () => {
          try {
            ...
            else reject(new Error(data.detail || t('settings.documents.upload.uploadFailed')));
          } catch {
            reject(new Error(t('settings.documents.upload.invalidResponse')));
          }
        });
        xhr.addEventListener("error", () => reject(new Error(t('settings.documents.upload.networkError'))));
      ...
      setError(err.message || t('settings.documents.upload.uploadFailed'));
    } finally {
      setUploading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-5 border-b border-gray-200 sticky top-0 bg-white">
          <h3 className="text-lg font-semibold text-gray-900">{t('settings.documents.upload.title')}</h3>
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('settings.documents.upload.docType')}</label>
            <select
              value={docType}
          ...
          {existingDoc && (
            <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-800">
              <strong>{t('common.note')}:</strong> {t('settings.documents.upload.existingDoc', { status: existingDoc.status, type: selectedTypeMeta?.label })}
            </div>
          )}

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('settings.documents.upload.file')}</label>
            <div
              onDrop={handleDrop}
            ...
            >
              ...
              {file ? (
                ...
                <>
                  ...
                  <p className="text-sm font-medium text-gray-700">{t('settings.documents.upload.dropOrBrowse')}</p>
                  <p className="text-xs text-gray-500 mt-1">{t('settings.documents.upload.fileTypes')}</p>
                </>
              )}
            </div>
          </div>
          ...
          {requiredFields.includes("document_number") && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('settings.documents.upload.docNumber')}</label>
              <input
                value={docNumber}
                onChange={e => setDocNumber(e.target.value)}
                className="w-full px-3 py-2 rounded-lg border border-gray-300 text-sm font-mono"
                placeholder={t('settings.documents.upload.docNumberPlaceholder')}
              />
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-3">
            {requiredFields.includes("issue_date") && (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('settings.documents.upload.issueDate')}</label>
                <input
                  type="date"
                  value={issueDate}
            ...
            {requiredFields.includes("expiry_date") && (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('settings.documents.upload.expiryDate')}</label>
                <input
                  type="date"
                  value={expiryDate}
          ...
          {requiredFields.includes("issuing_authority") && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('settings.documents.upload.issuingAuthority')}</label>
              <input
                value={issuingAuthority}
                onChange={e => setIssuingAuthority(e.target.value)}
                className="w-full px-3 py-2 rounded-lg border border-gray-300 text-sm"
                placeholder={t('settings.documents.upload.issuingAuthorityPlaceholder')}
              />
            </div>
          )}
          ...
          {uploading && (
            <div className="p-3 bg-[#8B1E3F]/5 border border-[#8B1E3F]/20 rounded-lg">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-medium text-[#8B1E3F]">{t('settings.documents.upload.uploading', { progress })}</span>
              </div>
        ...
        <div className="flex justify-end gap-3 p-5 border-t border-gray-200 sticky bottom-0 bg-white">
          <button onClick={onClose} disabled={uploading} className="px-4 py-2 text-sm border border-gray-300 rounded-lg hover:bg-gray-50">{t('common.cancel')}</button>
          <button
          ...
          >
            ...
            {t('settings.documents.upload.submit')}
          </button>
        </div>
      </div>
    </div>
