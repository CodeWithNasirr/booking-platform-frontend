...
export default function Sidebar({ sidebarOpen, setSidebarOpen }) {
  ...
  const go = (href) => {
    if (isDeactivated) return; // 🔥 Block navigation if deactivated
    router.push(href);
    setSidebarOpen(false);
  };

  const navItems = [
    { key: "/provider", label: "Home", icon: Home, disabled: isDeactivated },
    { key: "/provider/services", label: "My Services", icon: Briefcase, disabled: isDeactivated },
    { key: "/provider/orders", label: "Orders", icon: Package, badge: "2", disabled: isDeactivated },
    { key: "/provider/bookings", label: "Bookings", icon: Calendar, badge: "3", disabled: isDeactivated },
    { key: "/provider/availability", label: "Availability", icon: Clock, disabled: isDeactivated },
    { key: "/provider/work", label: "Work", icon: Wrench , disabled: false }, 

    // { key: "/provider/earnings", label: "Earnings", icon: DollarSign, disabled: isDeactivated },
    // { key: "/provider/chat", label: "Chat", icon: MessageSquare, disabled: isDeactivated },
    { key: "/provider/profile", label: "Profile", icon: User, disabled: false }, // Allow profile
    // { key: "/provider/settings", label: "Settings", icon: Settings, disabled: isDeactivated },
  ];
        ...
        {isDeactivated && (
          <div className="mx-4 mt-4 p-3 bg-red-50 border border-red-200 rounded-xl">
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-red-800 text-xs font-semibold">Account Deactivated</p>
                <p className="text-red-600 text-[10px] mt-0.5">Contact support for assistance</p>
              </div>
            </div>
          </div>
        )}
        ...
        {isPending && (
          <div className="mx-4 mt-4 p-3 bg-amber-50 border border-amber-200 rounded-xl">
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-amber-800 text-xs font-semibold">Pending Approval</p>
                <p className="text-amber-600 text-[10px] mt-0.5">Waiting for admin approval</p>
              </div>
            </div>
          </div>
        )}
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon
                  className={`w-5 h-5 ${isActive && !isDisabled ? "text-white" : "text-[#364153]"}`}
                  strokeWidth={1.66667}
                />
                <span className="font-medium text-[16px] leading-[24px]">{item.label}</span>
