// src/app/payment/hyperpay/callback/HyperPayCallbackClient.js
...
export default function HyperPayCallbackClient({
  checkoutId,
  type,
  refId,
}) {
    ...
    async function verifyPayment() {
      try {
        ...
        if (data.success) {
          ...
          setTimeout(() => {
            if (type === "booking") {
              router.push(`/booking/success?ref=${refId}`);
            } else if (type === "order") {
              router.push(`/order/success?ref=${refId}`);
          ...
          setStatus("failed");
          setError(
            data.result_description || "Payment was not successful."
          );
        }
      } catch {
        setStatus("failed");
