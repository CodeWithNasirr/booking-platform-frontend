...
export default function AdminSupportPage() {
  ...
  // status / priority maps — labels now come from t()
  const ST = useCallback(() => ({
    open:              { label: t("support_status_open"),          dot: "bg-blue-500",    pill: "bg-blue-50 text-blue-700" },
    in_progress:       { label: t("support_status_in_progress"),   dot: "bg-amber-500",   pill: "bg-amber-50 text-amber-700" },
    waiting_customer:  { label: t("support_status_waiting"),       dot: "bg-violet-500",  pill: "bg-violet-50 text-violet-700" },
    resolved:          { label: t("support_status_resolved"),      dot: "bg-emerald-500", pill: "bg-emerald-50 text-emerald-700" },
    closed:            { label: t("support_status_closed"),        dot: "bg-gray-400",    pill: "bg-gray-100 text-gray-600" },
  }), [t]);

  const PR = useCallback(() => ({
    low:    { label: t("support_priority_low"),    color: "text-gray-600",  bg: "bg-gray-100" },
    medium: { label: t("support_priority_medium"), color: "text-blue-600",  bg: "bg-blue-50" },
    high:   { label: t("support_priority_high"),   color: "text-amber-600", bg: "bg-amber-50" },
    urgent: { label: t("support_priority_urgent"), color: "text-red-600",   bg: "bg-red-50" },
  }), [t]);
  ...
  async function handleReply() {
    ...
    try {
      ...
      setMsgs(Array.isArray(thread) ? thread : thread?.results || []);
      setTimeout(() => endRef.current?.scrollIntoView({ behavior: "smooth" }), 80);
      if (!isInternal) flash(t("support_reply_sent"));
  ...
  async function doStatus(s) {
    setActionBusy(true);
    try {
      const u = await changeTicketStatus(active.id, s);
      setActive(u);
      load(true);
      flash(`→ ${ST()[s]?.label || s}`);
    } catch (e) { flash(e.message, "error"); }
  ...
  async function doPriority(p) {
    setActionBusy(true);
    try {
      const u = await changeTicketPriority(active.id, p);
      setActive(u);
      load(true);
    } catch (e) { flash(e.message, "error"); }
    setActionBusy(false);
  ...
  async function doAssign(agentId) {
    setAssigning(true);
    try {
      const u = await assignTicketTo(active.id, agentId);
      setActive(u);
      setShowAssign(false);
      load(true);
      flash(t("support_assigned"));
    } catch (e) { flash(e.message, "error"); }
    setAssigning(false);
  }
  async function doEscalate() {
    if (!confirm(t("support_escalate_confirm"))) return;
    setActionBusy(true);
    try {
      const u = await escalateTicketById(active.id, t("support_escalated_by_admin"));
      setActive(u);
      load(true);
      flash(t("support_escalated"));
    } catch (e) { flash(e.message, "error"); }
    setActionBusy(false);
  }
  ...
  return (
    <SuperAdminLayout
      ...
      breadcrumbs={[{ label: t("support_breadcrumb") }]}
    >
      ...
      {stats && (
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-2.5 mb-5">
          {[
            { label: t("support_stat_open"),         val: stats.open,             c: "text-blue-600" },
            { label: t("support_stat_in_progress"),  val: stats.in_progress,      c: "text-amber-600" },
            { label: t("support_stat_waiting"),       val: stats.waiting_customer, c: "text-violet-600" },
            { label: t("support_stat_unassigned"),    val: stats.unassigned,       c: "text-red-600" },
            { label: t("support_stat_resolved_today"), val: stats.resolved_today, c: "text-emerald-600" },
            { label: t("support_stat_sla_breached"),   val: stats.sla_breached,     c: "text-red-600" },
          ].map(s => (
            ...
            <div className="flex gap-1 flex-wrap">
              {["open", "in_progress", "waiting_customer", "resolved", "closed", ""].map(s => (
                <button key={s || "all"} onClick={() => setStatusF(s)}
                  ...
                  {s ? stMap[s]?.label : t("support_filter_all")}
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            ...
            <div className="text-center py-20">
              ...
              <p className="text-sm text-gray-400">{t("support_no_tickets")}</p>
            </div>
          ) : (
            tickets.map((ticket) => {
              const s = stMap[ticket.status] || stMap.open;
              const p = prMap[ticket.priority] || prMap.medium;
              const sel = active?.id === ticket.id;
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-2">
                    <span
                      className={`w-[7px] h-[7px] rounded-full mt-[7px] flex-shrink-0 ${s.dot}`}
                    />
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 mt-0.5">
                        ...
                        <span
                          className={`px-1 py-[1px] rounded text-[9px] font-semibold ${p.bg} ${p.color}`}
                        >
                          {ticket.priority}
                        </span>
          ...
          <div className="px-3 py-2 border-t border-gray-50 flex items-center justify-between">
            <span className="text-[10px] text-gray-400">{t("support_ticket_count", { count: tickets.length })}</span>
        ...
        <div className={`${active ? "flex" : "hidden lg:flex"} flex-col flex-1 min-w-0`}>
          {!active ? (
            <div className="flex-1 flex flex-col items-center justify-center text-gray-300">
              ...
              <p className="text-sm text-gray-400 font-medium">{t("support_select_ticket")}</p>
            </div>
          ) : (
            <>
              {/* ── Header + actions ── */}
                ...
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    ...
                    <select value={active.status} onChange={e => doStatus(e.target.value)} disabled={actionBusy}
                      ...
                      {Object.entries(stMap).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
                    </select>
                    {/* Priority */}
                    <select value={active.priority} onChange={e => doPriority(e.target.value)} disabled={actionBusy}
                      className="px-2 py-1 rounded-lg border border-gray-200 text-[11px] bg-white focus:outline-none cursor-pointer">
                      {Object.entries(prMap).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
                    </select>
                    {/* Assign */}
                    <div className="relative">
                      <button onClick={() => setShowAssign(!showAssign)}
                        ...
                        <span className="max-w-[60px] truncate">
                          {active.assigned_to_email ? active.assigned_to_email.split("@")[0] : t("support_assign")}
                        </span>
                      </button>
                      {showAssign && (
                        <>
                          ...
                          <div className="absolute right-0 top-full mt-1 z-20 w-52 bg-white border border-gray-200 rounded-xl shadow-xl py-1 max-h-52 overflow-y-auto">
                            ...
                            {staff.filter(s => s.is_active).length === 0 && (
                              <p className="px-3 py-2 text-xs text-gray-400">{t("support_no_staff")}</p>
                            )}
                          </div>
                        </>
                      )}
                    ...
                    {!active.is_escalated && (
                      <button onClick={doEscalate} disabled={actionBusy}
                        ...
                        <Zap className="w-3 h-3" /> {t("support_escalate")}
                      </button>
                    )}
                  </div>
                </div>
              ...
              <div className="px-4 py-1.5 bg-gray-50/60 border-b border-gray-50 flex items-center justify-between">
                <button onClick={() => setShowInternal(!showInternal)} className="flex items-center gap-1.5 text-[10px] text-gray-500 hover:text-gray-700">
                  ...
                  {showInternal ? t("support_internal_visible") : t("support_internal_hidden")}
                </button>
                <span className="text-[10px] text-gray-400">{t("support_message_count", { count: msgs.length })}</span>
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
                ...
                {active.description && (
                  <div className="bg-gray-50 rounded-xl p-4 mb-1 border border-gray-100">
                    <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1">{t("support_original_description")}</p>
                        ...
                        <div className={`max-w-[72%] rounded-2xl px-4 py-2.5 ${
                          ...
                          {internal && (
                            <div className="flex items-center gap-1 mb-1">
                              ...
                              <span className="text-[10px] font-bold text-amber-700">{t("support_internal_note")}</span>
                            </div>
                          )}
              ...
              <div className="px-4 py-3 border-t border-gray-100">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <button onClick={() => setIsInternal(true)}
                    ...
                    <Shield className="w-3 h-3" /> {t("support_internal_note")}
                  </button>
                </div>
                <div className="flex items-end gap-2">
                  <textarea
                    value={reply} onChange={e => setReply(e.target.value)}
                    placeholder={isInternal ? t("support_placeholder_internal") : t("support_placeholder_reply")}
                    rows={2}
