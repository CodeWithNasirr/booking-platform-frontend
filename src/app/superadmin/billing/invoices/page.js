...
function StatusBadge({ status, t }) {
  const INVOICE_STATUS = {
    paid:    { bg: "bg-emerald-50", text: "text-emerald-700", icon: CheckCircle, label: t("superadmin.billing.invoice_status_paid") },
    pending: { bg: "bg-amber-50",   text: "text-amber-700",   icon: Clock,      label: t("superadmin.billing.invoice_status_pending") },
    overdue: { bg: "bg-red-50",     text: "text-red-700",     icon: AlertCircle, label: t("superadmin.billing.invoice_status_overdue") },
    void:    { bg: "bg-gray-100",   text: "text-gray-600",    icon: XCircle,    label: t("superadmin.billing.invoice_status_void") },
  };
      ...
      <SuperAdminLayout>
        ...
        <div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">{t("superadmin.billing.invoices_title")}</h1>
              <p className="text-sm text-gray-500 mt-1">{t("superadmin.billing.invoices_desc")}</p>
            </div>
            <button
              className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50"
              title={t("superadmin.billing.export_tooltip")}
            >
              <Download className="w-4 h-4" /> {t("superadmin.billing.export")}
            </button>
          </div>
        </div>
        ...
        <div className="flex items-start gap-3 p-4 bg-amber-50 border border-amber-200 rounded-xl">
          ...
          <div>
            <p className="text-sm font-medium text-amber-800">{t("superadmin.billing.preview_mode_title")}</p>
            <p className="text-xs text-amber-700 mt-0.5"
              dangerouslySetInnerHTML={{
                __html: t("superadmin.billing.preview_mode_desc"),
              }}
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <div className="text-xs text-gray-500 mb-1">{t("superadmin.billing.stat_total_paid")}</div>
            <div className="text-2xl font-semibold text-emerald-600">{formatCurrency(totalPaid)}</div>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <div className="text-xs text-gray-500 mb-1">{t("superadmin.billing.stat_pending")}</div>
            <div className="text-2xl font-semibold text-amber-600">{formatCurrency(totalPending)}</div>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <div className="text-xs text-gray-500 mb-1">{t("superadmin.billing.stat_overdue")}</div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder={t("superadmin.billing.search_placeholder")}
            ...
            >
              ...
              <option value="paid">{t("superadmin.billing.invoice_status_paid")}</option>
              <option value="pending">{t("superadmin.billing.invoice_status_pending")}</option>
              <option value="overdue">{t("superadmin.billing.invoice_status_overdue")}</option>
            </select>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50 text-left">
                  <th className="px-5 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">{t("superadmin.billing.column_invoice")}</th>
                  ...
                  <th className="px-5 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">{t("superadmin.billing.column_amount")}</th>
                  <th className="px-5 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">{t("superadmin.billing.column_status")}</th>
                  <th className="px-5 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">{t("superadmin.billing.column_issued")}</th>
                  <th className="px-5 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">{t("superadmin.billing.column_paid")}</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {filtered.map((inv) => (
                  <tr key={inv.id} className="hover:bg-gray-50/60 transition-colors">
                    ...
                    <td className="px-5 py-3.5"><StatusBadge status={inv.status} t={t} /></td>
                    ...
                    <td className="px-5 py-3.5 text-right">
                      <button className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-600" title={t("superadmin.billing.download_pdf_tooltip")}>
          ...
          <div className="px-5 py-3 border-t border-gray-100 text-xs text-gray-500">
            {t("superadmin.billing.showing_results", { filtered: filtered.length, total: invoices.length, plural: invoices.length !== 1 ? "s" : "" })}
          </div>
        </div>
      </SuperAdminLayout>
    </div>
