...
import {
  ...
  Clock, Globe, ChevronLeft, ChevronRight, Loader2, X, AlertCircle,
} from "lucide-react";
...
function StatusBadge({ status, t }) {
  ...
  return (
    ...
    >
      {t(`superadmin.tenants.status_${status}`) || status}
    </span>
  );
}
...
// ─── Dropdown Menu ──────────────────────────────────────────────
function ActionDropdown({ tenant, onView, onEdit, onSuspend, onActivate, t, isRTL }) {
  ...
  const handleToggle = () => {
    if (!open && buttonRef.current) {
      ...
      setCoords({
        top,
        left: isRTL ? rect.left : rect.right - dropdownWidth,
        placement,
      });
    }
    setOpen(!open);
        ...
        <>
          ...
          >
            ...
            >
              ...
              {t("superadmin.tenants.action_view")}
            </button>
            <button
            ...
            >
              ...
              {t("superadmin.tenants.action_edit")}
            </button>
            ...
            {isActive ? (
              ...
              >
                ...
                <span className="text-amber-700">{t("superadmin.tenants.action_suspend")}</span>
              </button>
            ) : (
              <button
              ...
              >
                ...
                <span className="text-emerald-700">{t("superadmin.tenants.action_activate")}</span>
              </button>
            )}
          </div>
        </>
...
export default function TenantsListPage() {
  ...
  const loadTenants = async () => {
    ...
    try {
      ...
      showToast(t("superadmin.tenants.error_generic"), "error");
    }
    setLoading(false);
  };
  ...
  const handleSuspend = async (tenant) => {
    if (!window.confirm(t("superadmin.tenants.suspend_confirm_prompt", { name: tenant.name }))) return;
    setActionLoading(tenant.id);
    try {
      await suspendTenant(tenant.id, t("superadmin.tenants.suspend_default_reason"));
      loadTenants();
      loadStats();
      showToast(t("superadmin.tenants.suspend_success", { name: tenant.name }));
    } catch (err) {
      showToast(err.message || t("superadmin.tenants.error_generic"), "error");
    }
  ...
  const handleActivate = async (tenant) => {
    ...
    try {
      ...
      loadTenants();
      loadStats();
      showToast(t("superadmin.tenants.activate_success", { name: tenant.name }));
    } catch (err) {
      showToast(err.message || t("superadmin.tenants.error_generic"), "error");
    }
    setActionLoading(null);
  ...
  const handleEditSave = async (payload) => {
    ...
    try {
      ...
      loadTenants();
      loadStats();
      showToast(t("superadmin.tenants.update_success", { name: updated.name }));
  ...
  const openEditModal = async (tenant) => {
    try {
      ...
      showToast(t("superadmin.tenants.error_load_details"), "error");
    } finally {
      setActionLoading(null);
    }
  };
  ...
  // ── Status & Tier options for filters ──
  const statusOptions = [
    { value: "all", label: t("superadmin.tenants.filter_all_status") },
    { value: "active", label: t("superadmin.tenants.filter_active") },
    { value: "trial", label: t("superadmin.tenants.filter_trial") },
    { value: "suspended", label: t("superadmin.tenants.filter_suspended") },
    { value: "pending", label: t("superadmin.tenants.filter_pending") },
    { value: "cancelled", label: t("superadmin.tenants.filter_cancelled") },
  ];

  const tierOptions = [
    { value: "all", label: t("superadmin.tenants.filter_all_plans") },
    { value: "free", label: t("superadmin.tenants.filter_free") },
    { value: "starter", label: t("superadmin.tenants.filter_starter") },
    { value: "professional", label: t("superadmin.tenants.filter_professional") },
    { value: "enterprise", label: t("superadmin.tenants.filter_enterprise") },
  ];

  return (
    <SuperAdminLayout
      title={t("superadmin.tenants.title")}
      description={t("superadmin.tenants.description")}
      breadcrumbs={[{ label: t("superadmin.tenants.title") }]}
    >
      <div className="space-y-6" dir={isRTL ? "rtl" : "ltr"}>
        {/* Toast */}
        {toast && (
          <div className={`fixed top-4 ${isRTL ? "left-4" : "right-4"} z-50 flex items-center gap-2 px-4 py-3 rounded-lg shadow-lg text-sm font-medium text-white ${toast.type === "error" ? "bg-red-600" : "bg-emerald-600"}`}>
        ...
        {stats && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            <StatCard
              ...
              change={t("superadmin.stats.change_positive", { value: "12.5" })}
              positive
              icon={Building2}
            ...
            <StatCard
              ...
              change={t("superadmin.stats.change_positive", { value: "8.1" })}
              positive
              icon={CheckCircle}
            ...
            <StatCard
              ...
              change={t("superadmin.stats.change_positive", { value: "15.3" })}
              positive
              icon={Clock}
              gradient="linear-gradient(135deg, #D97706 0%, #B45309 100%)"
            />
            <StatCard
              label={t("superadmin.stats.suspended")}
              value={stats.suspended?.toLocaleString() || "0"}
              change={t("superadmin.stats.change_negative", { value: "-5.2" })}
              positive={false}
              icon={Ban}
          ...
          <div className="flex flex-col lg:flex-row gap-3">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className={`w-full h-11 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:border-transparent transition-shadow ${isRTL ? "pr-10 pl-10" : "pl-10 pr-10"}`}
        ...
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
          {loading ? (
            <div className="flex items-center justify-center py-20">
              ...
              <span className="ml-2 text-sm text-gray-500">{t("superadmin.tenants.loading")}</span>
            </div>
          ) : tenants.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-gray-400">
              <Building2 className="w-12 h-12 mb-3" />
              <p className="text-sm font-medium">{t("superadmin.tenants.no_results")}</p>
              <p className="text-xs mt-1">{t("superadmin.tenants.no_results_hint")}</p>
            </div>
          ) : (
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gray-50/80 border-b border-gray-200">
                    <th className="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{t("superadmin.tenants.column_tenant")}</th>
                    <th className="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{t("superadmin.tenants.column_plan")}</th>
                    <th className="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{t("superadmin.tenants.column_status")}</th>
                    <th className="px-6 py-3.5 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">{t("superadmin.tenants.column_members")}</th>
                    <th className="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{t("superadmin.tenants.column_domain")}</th>
                    <th className="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{t("superadmin.tenants.column_created")}</th>
                    <th className="px-6 py-3.5 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">{t("superadmin.tenants.column_actions")}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {tenants.map((tenant) => (
                    <tr
                      key={tenant.id}
                      className={`hover:bg-gray-50/60 transition-colors ${actionLoading === tenant.id ? "opacity-50" : ""}`}
                    >
                      ...
                      <td className="px-6 py-4">
                        <TierBadge tier={tenant.subscription_tier} t={t} />
                      </td>
                      <td className="px-6 py-4">
          ...
          {!loading && tenants.length > 0 && (
            <div className="flex items-center justify-between px-6 py-4 border-t border-gray-200 bg-gray-50/50">
              <p className="text-sm text-gray-600">
                {t("superadmin.tenants.pagination_showing", {
                  from: (page - 1) * 15 + 1,
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">
              {t("superadmin.tenants.suspend_confirm_title", { name: selectedTenant.name })}
            </h3>
            <p className="text-sm text-gray-600 mb-6">
              {t("superadmin.tenants.suspend_confirm_desc")}
            </p>
            <div className="flex justify-end gap-3">
              ...
              >
                {actionLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : t("superadmin.tenants.suspend")}
              </button>
            </div>
          </div>
        </div>
      ...
      {showActivateModal && selectedTenant && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">
              {t("superadmin.tenants.activate_confirm_title", { name: selectedTenant.name })}
            </h3>
            <p className="text-sm text-gray-600 mb-6">
              {t("superadmin.tenants.activate_confirm_desc")}
            </p>
            <div className="flex justify-end gap-3">
              <button
              ...
              >
                {actionLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : t("superadmin.tenants.activate")}
              </button>
            </div>
          </div>
        </div>
