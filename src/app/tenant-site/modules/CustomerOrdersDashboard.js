...
export default function CustomerOrdersDashboard({
  ...
  useEffect(() => {
    ...
    // Check for stored guest access token (keyed by tenantId)
    if (tenantId) {
      const stored = localStorage.getItem(tokenKey(tenantId));
      if (stored) {
        setAccessToken(stored);
        setIsGuestToken(true);
        setCustomerEmail(localStorage.getItem(emailStoreKey(tenantId)) || "");
        setAuthState("authenticated");
        return;
      }
    }
  ...
  }, [accessToken, isGuestToken, domain, tenantId]);
  ...
  const handleVerifyOTP = async (e) => {
    ...
    try {
      ...
      if (tenantId) {
        localStorage.setItem(tokenKey(tenantId), newToken);
        localStorage.setItem(emailStoreKey(tenantId), customerEmail);
      }
