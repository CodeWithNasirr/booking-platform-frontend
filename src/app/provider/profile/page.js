...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      try {
        ...
        toast.error(t('profile_error_load'));
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(t('profile_error_update'));
  ...
  const handleToggleService = async (serviceId) => {
    try {
      ...
      if (!response.ok) throw new Error(t('profile_error_toggle'));
  ...
  const handleAddService = async (serviceId) => {
    try {
      ...
      if (!response.ok) throw new Error(t('profile_error_add_service'));
      
      const data = await response.json();
      setServices([...services, data.service]);
      toast.success(t('profile_toast_service_added'));
    } catch (err) {
      toast.error(err.message);
    }
  };
  ...
  const handleSaveNotifications = async () => {
    try {
      ...
      if (!response.ok) throw new Error(t('profile_error_save_notif'));
      
      toast.success(t('profile_toast_notif_saved'));
    } catch (err) {
      toast.error(err.message);
    }
  };
  ...
  const handleChangePassword = async (e) => {
    ...
    if (data.new_password !== data.confirm_password) {
      toast.error(t('profile_error_password_match'));
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(err.detail || t('profile_error_password_change'));
      }
      
      toast.success(t('profile_toast_password_changed'));
      e.target.reset();
    } catch (err) {
      toast.error(err.message);
    }
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(t('profile_error_delete'));
      
      toast.success(t('profile_toast_deleted'));
  ...
  if (loading) {
    return (
      <DashboardLayout pageName={t('profile_page_title')}>
  ...
  const tabs = [
    { id: 'profile', label: t('profile_tab_profile'), icon: User },
    { id: 'services', label: t('profile_tab_services'), icon: Briefcase },
    { id: 'notifications', label: t('profile_tab_notifications'), icon: Bell },
  ...
  const statCards = [
    { 
      icon: Star, 
      label: t('profile_stat_rating'), 
      value: `${stats.rating}/5.0`, 
      subtext: t('profile_stat_reviews', { count: stats.total_reviews }), 
      color: 'text-[#f59e0b]', 
      bg: 'bg-[#fffbeb]', 
    ...
    { 
      icon: CheckCircle, 
      label: t('profile_stat_completed'), 
      value: stats.completed_jobs.toString(), 
      subtext: t('profile_stat_jobs'), 
      color: 'text-[#2463eb]', 
      bg: 'bg-[#eff6ff]', 
    ...
    { 
      icon: ShieldCheck, 
      label: t('profile_stat_success'), 
      value: `${stats.success_rate}%`, 
      subtext: t('profile_stat_completion'), 
      color: 'text-[#16a34a]', 
      bg: 'bg-[#f0fdf4]', 
  ...
  const notificationOptions = {
    new_bookings: { 
      title: t('profile_notif_new_bookings_title'), 
      desc: t('profile_notif_new_bookings_desc') 
    },
    booking_confirmations: { 
      title: t('profile_notif_booking_conf_title'), 
      desc: t('profile_notif_booking_conf_desc') 
    },
    order_updates: { 
      title: t('profile_notif_order_updates_title'), 
      desc: t('profile_notif_order_updates_desc') 
    },
    messages: { 
      title: t('profile_notif_messages_title'), 
      desc: t('profile_notif_messages_desc') 
    },
    promotions: { 
      title: t('profile_notif_promotions_title'), 
      desc: t('profile_notif_promotions_desc') 
    },
    service_requests: { 
      title: t('profile_notif_service_req_title'), 
      desc: t('profile_notif_service_req_desc') 
    }
  };
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4">
                <div>
                  ...
                  <p className="text-[14px] text-[#4a5565] leading-[20px]">
                    {profile.profession || t('profile_default_profession')} • {profile.email}
                  </p>
                </div>
                <div className="bg-[#dcfce7] h-8 px-4 rounded-[10px] border border-[#bbf7d0] flex items-center w-fit">
                  <span className="text-[14px] text-[#166534] font-medium">{t('profile_status_active')}</span>
                </div>
              </div>
        ...
        <div className="min-h-[400px]">
          {activeTab === 'profile' && (
            <div className="flex flex-col gap-6 max-w-[800px]">
              <div>
                <h3 className="text-[16px] md:text-[18px] text-[#101828] font-semibold leading-[24px] mb-4">
                  {t('profile_section_personal')}
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex flex-col gap-2">
                    <label className="text-[14px] text-[#364153] leading-[20px] font-medium">{t('profile_label_name')}</label>
                  ...
                  <div className="flex flex-col gap-2">
                    <label className="text-[14px] text-[#364153] leading-[20px] font-medium">{t('profile_label_email')}</label>
                    <div className="relative">
                  ...
                  <div className="flex flex-col gap-2">
                    <label className="text-[14px] text-[#364153] leading-[20px] font-medium">{t('profile_label_phone')}</label>
                    <div className="relative">
                  ...
                  <div className="flex flex-col gap-2">
                    <label className="text-[14px] text-[#364153] leading-[20px] font-medium">{t('profile_label_profession')}</label>
                    <div className="relative">
                  ...
                  <div className="flex flex-col gap-2 md:col-span-2">
                    <label className="text-[14px] text-[#364153] leading-[20px] font-medium">{t('profile_label_bio')}</label>
                    <textarea
                      value={profile.bio}
          ...
          {activeTab === 'services' && (
            <div className="flex flex-col gap-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <h3 className="text-[16px] md:text-[18px] text-[#101828] font-semibold leading-[24px]">
                    {t('profile_section_services')}
                  </h3>
                  <p className="text-[14px] text-[#4a5565] leading-[20px]">
                    {t('profile_services_desc')}
                  </p>
                </div>
                <button 
          ...
          {activeTab === 'add-service' && (
            <div className="flex flex-col gap-6">
              <div className="flex items-center gap-4 mb-4">
                ...
                <h3 className="text-[16px] md:text-[18px] text-[#101828] font-semibold leading-[24px]">
                  {t('profile_add_service_title')}
                </h3>
              </div>
              
              <div className="flex flex-col gap-3">
                {availableServices.length === 0 ? (
                  <p className="text-[#4a5565]">{t('profile_no_services')}</p>
                ) : (
                  availableServices.map((service) => (
                    <div key={service.id} className="bg-white border border-[#e5e7eb] rounded-[12px] p-4 flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-[14px] text-[#4a5565]">{service.duration_minutes} {t('profile_min')} • {t('profile_from')} ${service.base_price}</p>
                      </div>
                      <button 
          ...
          {activeTab === 'notifications' && (
            <div className="flex flex-col gap-6 max-w-[800px]">
              <div>
                <h3 className="text-[16px] md:text-[18px] text-[#101828] font-semibold leading-[24px] mb-2">
                  {t('profile_section_notifications')}
                </h3>
                <p className="text-[14px] text-[#4a5565] leading-[20px]">
                  {t('profile_notifications_desc')}
                </p>
              </div>

              <div className="flex flex-col">
                {Object.entries(notificationOptions).map(([key, { title, desc }]) => (
          ...
          {activeTab === 'security' && (
            <div className="flex flex-col gap-6 max-w-[600px]">
              <div>
                ...
                <p className="text-[14px] text-[#4a5565] leading-[20px] mb-6">
                  {t('profile_password_desc')}
                </p>
                
                <form onSubmit={handleChangePassword} className="flex flex-col gap-4">
                  <div className="flex flex-col gap-2">
                    <label className="text-[14px] text-[#364153] leading-[20px] font-medium">{t('profile_label_current_password')}</label>
                    <input
                  ...
                  <div className="flex flex-col gap-2">
                    <label className="text-[14px] text-[#364153] leading-[20px] font-medium">{t('profile_label_new_password')}</label>
                    <input
                      name="new_password"
                      type="password"
                      required
                  ...
                  <div className="flex flex-col gap-2">
                    <label className="text-[14px] text-[#364153] leading-[20px] font-medium">{t('profile_label_confirm_password')}</label>
                    <input
              ...
              <div className="pt-6 border-t border-[#f3f4f6]">
                <h3 className="text-[16px] font-semibold leading-[24px] mb-4 text-red-600">
                ...
                <div className="bg-red-50 border border-red-100 rounded-[12px] p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    <h4 className="text-[14px] text-[#101828] font-medium leading-[20px] mb-1">{t('profile_delete_title')}</h4>
                    <p className="text-[14px] text-[#4a5565] leading-[20px]">{t('profile_delete_desc')}</p>
                  </div>
                  <button 
