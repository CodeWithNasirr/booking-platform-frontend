...
/**
 * ProviderOrderDetailClient — Provider Order Detail View (Fully i18n)
 */
...
const STATUS_CONFIG = {
  ...
  paid:               { labelKey: "orders_status_paid",               color: "bg-blue-50 text-blue-700 border border-blue-200",        icon: "🔔" },
  accepted:           { labelKey: "orders_status_accepted",           color: "bg-indigo-50 text-indigo-700 border border-indigo-200",  icon: "✓" },
  in_progress:        { labelKey: "orders_status_in_progress",        color: "bg-violet-50 text-violet-700 border border-violet-200",  icon: "⚡" },
  delivered:          { labelKey: "orders_status_delivered",          color: "bg-teal-50 text-teal-700 border border-teal-200",       icon: "📦" },
  completed:          { labelKey: "orders_status_completed",          color: "bg-emerald-50 text-emerald-700 border border-emerald-200", icon: "✅" },
  revision_requested: { labelKey: "orders_status_revision_requested", color: "bg-orange-50 text-orange-700 border border-orange-200",  icon: "🔄" },
  cancelled:          { labelKey: "orders_status_cancelled",          color: "bg-red-50 text-red-700 border border-red-200",           icon: "✕" },
  refunded:           { labelKey: "orders_status_refunded",           color: "bg-gray-50 text-gray-600 border border-gray-200",        icon: "↩" },
};

const PROVIDER_ACTIONS = {
  accepted:           [{ endpoint: "start_work", labelKey: "orders_action_start_work",      icon: "🚀", needsMessage: false }],
  in_progress:        [{ endpoint: "deliver",    labelKey: "orders_action_deliver",         icon: "📦", needsMessage: true }],
  revision_requested: [{ endpoint: "deliver",    labelKey: "orders_action_resubmit",        icon: "📦", needsMessage: true }],
};

export default function ProviderOrderDetailClient({ orderId }) {
  const router = useRouter();
  const { t, language, activeTenant, tenants, user, isRTL } = useApp();
  ...
  const fetchOrder = useCallback(async () => {
    try {
      ...
      if (err.status === 404) { setError(t("orders_not_found")); return; }
      setError(err.message || t("orders_error_load"));
    } finally {
      setLoading(false);
    }
  }, [orderId, tenantId, router, t]);
  ...
  const handleAction = async (actionDef) => {
    ...
    try {
      ...
      alert(err.message || t("orders_action_failed"));
    } finally {
  ...
  if (error || !order) {
    return (
      <div className="max-w-4xl mx-auto p-6 text-center py-20">
        <p className="text-red-600 text-lg mb-4">{error || t("orders_not_found")}</p>
        <div className="flex gap-3 justify-center">
          <button onClick={fetchOrder} className="text-blue-600 hover:underline text-sm">
            {t("orders_try_again")}
          </button>
          <button onClick={() => router.push("/provider/orders")} className="text-gray-500 hover:underline text-sm">
            {t("orders_back")}
          </button>
        </div>
      </div>
        ...
        <div className="col-span-12 lg:col-span-8 space-y-5">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <div className="flex flex-col sm:flex-row justify-between items-start gap-3">
              <div>
                <h1 className="text-xl font-bold text-gray-900">{order.service_name || t("orders_order")}</h1>
                <p className="text-sm text-gray-400 mt-0.5 font-mono">#{order.order_number}</p>
                {order.package_name && (
                  <p className="text-xs text-gray-400 mt-1">{t("orders_package")}: {order.package_name}</p>
                )}
              </div>
              <div className="flex items-center gap-2">
                ...
                <span className={`px-3 py-1.5 rounded-lg text-xs font-semibold ${sc.color}`}>
                  {sc.icon} {t(sc.labelKey) || order.status}
                </span>
              </div>
            </div>

            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-5 pt-4 border-t border-gray-100">
              <QuickStat label={t("orders_customer")} value={order.customer_name_display || order.customer_name || "—"} />
              <QuickStat label={t("orders_ordered")} value={new Date(order.created_at).toLocaleDateString()} />
              {order.due_date && (
                <QuickStat label={t("orders_due_date")} value={new Date(order.due_date).toLocaleDateString()} />
              )}
              <QuickStat 
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                <p className="text-[11px] text-gray-400">{t("orders_total")}</p>
                <p className="text-lg font-bold text-gray-800">
                  ${parseFloat(order.total_amount || 0).toFixed(2)}
                </p>
              </div>
              <div>
                <p className="text-[11px] text-gray-400">{t("orders_platform_fee")}</p>
                <p className="text-lg font-bold text-red-500">
                  -${parseFloat(order.platform_fee || 0).toFixed(2)}
                </p>
              </div>
              <div>
                <p className="text-[11px] text-gray-400">{t("orders_you_earn")}</p>
            ...
            >
              ...
              {showDeliveryForm ? (
                <div className="space-y-3">
                  <h3 className="font-semibold text-gray-800">{t("orders_deliver")}</h3>
                  <p className="text-sm text-gray-500">{t("orders_delivery_desc")}</p>
                  <textarea
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      {actionLoading === "deliver" ? `${t("orders_submitting")}...` : `📦 ${t("orders_deliver")}`}
                    </button>
                    <button
                ...
                <div className="flex flex-wrap gap-2">
                  {actions.map((a) => (
                    <button
                      ...
                      style={{ backgroundColor: primaryColor }}
                    >
                      {actionLoading === a.endpoint ? "..." : `${a.icon} ${t(a.labelKey)}`}
                    </button>
                  ))}
                </div>
              )}
          ...
          {order.review && (
            <div className="bg-amber-50/50 rounded-xl border border-amber-100 p-5">
              ...
              <div className="flex items-center gap-1.5 mb-2">
                {[1, 2, 3, 4, 5].map((s) => (
                  <span key={s} className={`text-lg ${s <= order.review.rating ? "text-amber-400" : "text-gray-200"}`}>
                    ★
                  </span>
                ))}
