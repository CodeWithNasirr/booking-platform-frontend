...
export default function AvailabilityPage() {
  ...
  const [days, setDays] = useState([
    { name: t('availability_day_monday'), day_of_week: 0, available: true, slots: [{ start: '09:00', end: '17:00' }] },
    { name: t('availability_day_tuesday'), day_of_week: 1, available: true, slots: [{ start: '09:00', end: '17:00' }] },
    { name: t('availability_day_wednesday'), day_of_week: 2, available: true, slots: [{ start: '09:00', end: '17:00' }] },
    { name: t('availability_day_thursday'), day_of_week: 3, available: true, slots: [{ start: '09:00', end: '17:00' }] },
    { name: t('availability_day_friday'), day_of_week: 4, available: true, slots: [{ start: '09:00', end: '17:00' }] },
    { name: t('availability_day_saturday'), day_of_week: 5, available: false, slots: [] },
    { name: t('availability_day_sunday'), day_of_week: 6, available: false, slots: [] },
  ]);
  ...
  const fetchAvailability = useCallback(async () => {
    try {
      ...
      if (!response.ok) throw new Error(t('availability_error_fetch'));

      const data = await response.json();
      if (data && data.length === 7) {
        // Map backend day names to localized names
        const localizedDays = data.map((day, index) => ({
          ...day,
          name: getLocalizedDayName(index, t),
        }));
        setDays(localizedDays);
      }
    } catch (err) {
  ...
  const fetchExceptions = useCallback(async () => {
    try {
      ...
      if (!response.ok) throw new Error(t('availability_error_fetch_exceptions'));
  ...
  const saveAvailability = async (updatedDays) => {
    try {
      ...
      if (!response.ok) throw new Error(t('availability_error_save'));

      toast.success(t('availability_toast_saved'));
      setHasChanges(false);
    } catch (err) {
      toast.error(err.message);
    } finally {
  ...
  const handleAddException = async (exceptionData) => {
    try {
      ...
      if (!response.ok) throw new Error(t('availability_error_add_exception'));

      toast.success(t('availability_toast_exception_added'));
      fetchExceptions();
  ...
  const deleteException = async (exceptionId) => {
    if (!confirm(t('availability_delete_confirm'))) return;
    ...
    try {
      ...
      if (!response.ok) throw new Error(t('availability_error_delete'));

      toast.success(t('availability_toast_deleted'));
      fetchExceptions();
    } catch (err) {
      toast.error(err.message);
    }
  ...
  if (loading) {
    return (
      <DashboardLayout pageName={t('availability_page_title')}>
    ...
    <DashboardLayout pageName={t('availability_page_title')}>
      <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
          <div>
            <h1 className="text-xl md:text-[24px] text-[#101828] font-bold leading-[32px]">
              {t('availability_page_title')}
            </h1>
            <p className="text-sm md:text-[16px] text-[#4a5565] leading-[24px]">
              {t('availability_page_subtitle')}
              {saving && <span className="ml-2 text-[#800020]">({t('availability_saving')})</span>}
            </p>
          </div>
          <button
        ...
        <div className="bg-[#f3f4f6] flex gap-2 p-1 rounded-[16px] w-fit">
          ...
          >
            <span className="text-[14px] md:text-[16px]">{t('availability_tab_weekly')}</span>
          </button>
          <button
          ...
          >
            <span className="text-[14px] md:text-[16px]">{t('availability_tab_exceptions')}</span>
          </button>
        </div>
        ...
        {activeTab === 'weekly' ? (
          <div className="flex flex-col gap-3 md:gap-4">
            {days.map((day, index) => (
              <div key={day.day_of_week} className="bg-white border border-[#e5e7eb] rounded-[16px] p-4 md:p-6">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-[14px] text-[#4a5565]">
                          {day.available ? t('availability_status_available') : t('availability_status_unavailable')}
                        </p>
                      </div>
                    </div>
                  </div>
                        ...
                        <div key={slotIndex} className="flex items-center gap-2">
                          ...
                          {day.slots.length > 1 && (
                            <button
                              ...
                              title={t('availability_remove_slot')}
                            >
                              <Trash2 size={16} />
                            </button>
                          )}
            ...
            {hasChanges && (
              ...
              >
                {saving ? t('availability_saving') : t('availability_save_changes')}
              </button>
            )}
          </div>
        ) : (
          <div className="flex flex-col gap-4">
            <div className="bg-[#eff6ff] border border-[#bfdbfe] rounded-[12px] p-4 flex gap-3">
              ...
              <div>
                <h3 className="text-[14px] text-[#1e40af] font-semibold mb-1">{t('availability_info_title')}</h3>
                <p className="text-[14px] text-[#1e40af] leading-[20px]">
                  {t('availability_info_desc')}
                </p>
              </div>
            </div>
                ...
                {exceptions.map((exc) => (
                  <div key={exc.id} className="bg-white border border-[#e5e7eb] rounded-[16px] p-4 flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-[14px] text-[#4a5565]">
                        {exc.is_available ? t('availability_exception_available') : t('availability_exception_unavailable')} 
...
// Helper to get localized day names by index
function getLocalizedDayName(index, t) {
  const dayKeys = [
    'availability_day_monday',
    'availability_day_tuesday',
    'availability_day_wednesday',
    'availability_day_thursday',
    'availability_day_friday',
    'availability_day_saturday',
    'availability_day_sunday',
  ];
