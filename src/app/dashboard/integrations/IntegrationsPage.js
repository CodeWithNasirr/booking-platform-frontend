...
export default function IntegrationsPage() {
  ...
  const CATEGORIES = [
    { key: 'all', labelKey: 'integrations.category.all', icon: Globe },
    { key: 'calendar', labelKey: 'integrations.category.calendar', icon: Calendar },
    { key: 'communication', labelKey: 'integrations.category.communication', icon: MessageCircle },
    { key: 'marketing', labelKey: 'integrations.category.marketing', icon: BarChart3 },
  ]
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-[#8B1E3F]/10 p-5 shadow-sm">
        <div className="flex items-center justify-between mb-3">
          <span className="text-sm font-bold text-gray-700">{t('integrations.statusBar.title')}</span>
          <span className="text-sm text-gray-500">{t('integrations.statusBar.count', { connected: connectedCount, total: totalCount })}</span>
        </div>
        ...
        <div className="flex items-center gap-6 mt-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-gray-600">{t('integrations.statusBar.active', { count: connectedCount })}</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-2.5 h-2.5 rounded-full bg-gray-300" />
            <span className="text-xs text-gray-600">{t('integrations.statusBar.available', { count: totalCount - connectedCount })}</span>
          </div>
        </div>
      </div>
      ...
      {activeModal?.type === 'google_calendar' && (
        <GoogleCalendarModal activeTenant={activeTenant} onClose={() => setActiveModal(null)} onRefresh={loadIntegrations} t={t} />
      )}
      {activeModal?.type === 'whatsapp_web' && (
        <WhatsAppModal activeTenant={activeTenant} onClose={() => setActiveModal(null)} onRefresh={loadIntegrations} t={t} />
      )}
      {activeModal?.type === 'pixel_config' && (
        <PixelConfigModal activeTenant={activeTenant} integration={activeModal.integration} onClose={() => setActiveModal(null)} onRefresh={loadIntegrations} t={t} />
      )}
      {activeModal?.type === 'disconnect' && (
        <DisconnectModal activeTenant={activeTenant} integration={activeModal.integration} onClose={() => setActiveModal(null)} onRefresh={loadIntegrations} t={t} />
      )}
      {activeModal?.type === 'setup_guide' && (
        <SetupGuideModal integration={activeModal.integration} onClose={() => setActiveModal(null)} t={t} />
      )}
    </div>
  )
}
...
function IntegrationCard({ integration, onAction, t, canManage }) {
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              ...
              <StatusDot status={integration.status} t={t} />
            </div>
        ...
        <div className="mx-5 mt-3 px-3 py-2 rounded-lg bg-green-50/80 border border-green-100">
          ...
          {integration.config?.connected_providers?.length > 0 && (
            <div className="flex items-center gap-2 text-sm text-green-700">
              ...
              {t('integrations.card.providersSynced', { count: integration.config.connected_providers.length })}
            </div>
          )}
          ...
          {integration.connected_at && (
            <p className="text-[10px] text-green-600 mt-1">
              {t('integrations.card.connectedAt', { date: new Date(integration.connected_at).toLocaleDateString() })}
            </p>
          )}
        </div>
      )}
      ...
      <div className="p-5 pt-3">
        ...
        {isPlatformDisabled ? (
          <div className="flex-1 flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl border border-gray-200 bg-gray-50 text-gray-500 text-sm font-medium cursor-not-allowed">
            ...
            {t('integrations.card.disabledByPlatform')}
          </div>
        ) : isConnected ? (
          <>
            <button
          ...
          <>
            ...
            <button
              ...
              title={t('integrations.card.setupGuide')}
            >
...
function StatusDot({ status, t }) {
  const cfg = {
    connected:    { color: 'bg-green-500',  ring: 'ring-green-200',  labelKey: 'integrations.status.connected' },
    pending:      { color: 'bg-amber-400',  ring: 'ring-amber-200',  labelKey: 'integrations.status.pending' },
    qr_ready:     { color: 'bg-blue-400',   ring: 'ring-blue-200',   labelKey: 'integrations.status.qrReady' },
    error:        { color: 'bg-red-500',     ring: 'ring-red-200',    labelKey: 'integrations.status.error' },
    disconnected: { color: 'bg-gray-300',    ring: 'ring-gray-100',   labelKey: 'integrations.status.disconnected' },
    disabled_by_platform: {
        color: 'bg-gray-400',
        ring: 'ring-gray-200',
        labelKey: 'integrations.status.disabledByPlatform',
      },
  }[status] || { color: 'bg-gray-300', ring: 'ring-gray-100', labelKey: status }

  return (
    <div className="flex items-center gap-1.5 flex-shrink-0" title={t(cfg.labelKey)}>
      <div className={`w-2 h-2 rounded-full ${cfg.color} ring-2 ${cfg.ring}`} />
      <span className="text-[11px] font-medium text-gray-500">{t(cfg.labelKey)}</span>
    </div>
  )
}
...
function SetupGuideModal({ integration, onClose, t }) {
  const guide = getSetupGuide(integration.id, t)
  if (!guide) {
    return (
      <Modal title={t('integrations.guide.title')} onClose={onClose}>
        <p className="text-sm text-gray-600 py-8 text-center">{t('integrations.guide.noGuide')}</p>
      </Modal>
    )
  }

  return (
    <Modal title={guide.title} subtitle={t('integrations.guide.subtitle')} onClose={onClose}>
      <div className="space-y-5">
        ...
        {guide.requirements?.length > 0 && (
          <div className="p-4 rounded-xl bg-amber-50 border border-amber-200">
            <p className="text-xs font-bold text-amber-800 uppercase tracking-wider mb-2">{t('integrations.guide.requirements')}</p>
        ...
        {guide.tracked_events?.length > 0 && (
          <div>
            <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">{t('integrations.guide.trackedEvents')}</p>
            <div className="flex flex-wrap gap-1.5">
        ...
        {guide.docs && (
          <a href={guide.docs} target="_blank" rel="noopener noreferrer"
            ...
            <ExternalLink className="w-4 h-4" /> {t('integrations.guide.viewDocs')}
          </a>
        )}
      </div>
    </Modal>
...
// Setup guides factory — returns translated content
function getSetupGuide(id, t) {
  const guides = {
    google_calendar: {
      title: t('integrations.guide.googleCalendar.title'),
      steps: [
        { title: t('integrations.guide.googleCalendar.step1.title'), desc: t('integrations.guide.googleCalendar.step1.desc') },
        { title: t('integrations.guide.googleCalendar.step2.title'), desc: t('integrations.guide.googleCalendar.step2.desc') },
        { title: t('integrations.guide.googleCalendar.step3.title'), desc: t('integrations.guide.googleCalendar.step3.desc') },
      ],
      requirements: [
        t('integrations.guide.googleCalendar.req1'),
        t('integrations.guide.googleCalendar.req2'),
      ],
      docs: 'https://support.google.com/calendar',
    },
    zoom: {
      title: t('integrations.guide.zoom.title'),
      steps: [
        { title: t('integrations.guide.zoom.step1.title'), desc: t('integrations.guide.zoom.step1.desc') },
        { title: t('integrations.guide.zoom.step2.title'), desc: t('integrations.guide.zoom.step2.desc') },
        { title: t('integrations.guide.zoom.step3.title'), desc: t('integrations.guide.zoom.step3.desc') },
      ],
      requirements: [
        t('integrations.guide.zoom.req1'),
        t('integrations.guide.zoom.req2'),
      ],
      docs: 'https://marketplace.zoom.us',
    },
    whatsapp_web: {
      title: t('integrations.guide.whatsapp.title'),
      steps: [
        { title: t('integrations.guide.whatsapp.step1.title'), desc: t('integrations.guide.whatsapp.step1.desc') },
        { title: t('integrations.guide.whatsapp.step2.title'), desc: t('integrations.guide.whatsapp.step2.desc') },
        { title: t('integrations.guide.whatsapp.step3.title'), desc: t('integrations.guide.whatsapp.step3.desc') },
        { title: t('integrations.guide.whatsapp.step4.title'), desc: t('integrations.guide.whatsapp.step4.desc') },
      ],
      requirements: [
        t('integrations.guide.whatsapp.req1'),
        t('integrations.guide.whatsapp.req2'),
      ],
      docs: 'https://faq.whatsapp.com/web',
    },
    meta_pixel: {
      title: t('integrations.guide.metaPixel.title'),
      steps: [
        { title: t('integrations.guide.metaPixel.step1.title'), desc: t('integrations.guide.metaPixel.step1.desc') },
        { title: t('integrations.guide.metaPixel.step2.title'), desc: t('integrations.guide.metaPixel.step2.desc') },
        { title: t('integrations.guide.metaPixel.step3.title'), desc: t('integrations.guide.metaPixel.step3.desc') },
      ],
      requirements: [
        t('integrations.guide.metaPixel.req1'),
        t('integrations.guide.metaPixel.req2'),
      ],
      docs: 'https://business.facebook.com/events_manager',
      tracked_events: ['PageView', 'ViewContent', 'AddToCart', 'InitiateCheckout', 'Purchase'],
    },
    google_analytics: {
      title: t('integrations.guide.googleAnalytics.title'),
      steps: [
        { title: t('integrations.guide.googleAnalytics.step1.title'), desc: t('integrations.guide.googleAnalytics.step1.desc') },
        { title: t('integrations.guide.googleAnalytics.step2.title'), desc: t('integrations.guide.googleAnalytics.step2.desc') },
        { title: t('integrations.guide.googleAnalytics.step3.title'), desc: t('integrations.guide.googleAnalytics.step3.desc') },
      ],
      requirements: [
        t('integrations.guide.googleAnalytics.req1'),
      ],
      docs: 'https://analytics.google.com',
      tracked_events: ['page_view', 'purchase', 'begin_checkout', 'view_item'],
    },
    google_tag_manager: {
      title: t('integrations.guide.googleTagManager.title'),
      steps: [
        { title: t('integrations.guide.googleTagManager.step1.title'), desc: t('integrations.guide.googleTagManager.step1.desc') },
        { title: t('integrations.guide.googleTagManager.step2.title'), desc: t('integrations.guide.googleTagManager.step2.desc') },
        { title: t('integrations.guide.googleTagManager.step3.title'), desc: t('integrations.guide.googleTagManager.step3.desc') },
      ],
      requirements: [
        t('integrations.guide.googleTagManager.req1'),
      ],
      docs: 'https://tagmanager.google.com',
    },
    tiktok_pixel: {
      title: t('integrations.guide.tiktokPixel.title'),
      steps: [
        { title: t('integrations.guide.tiktokPixel.step1.title'), desc: t('integrations.guide.tiktokPixel.step1.desc') },
        { title: t('integrations.guide.tiktokPixel.step2.title'), desc: t('integrations.guide.tiktokPixel.step2.desc') },
        { title: t('integrations.guide.tiktokPixel.step3.title'), desc: t('integrations.guide.tiktokPixel.step3.desc') },
      ],
      requirements: [
        t('integrations.guide.tiktokPixel.req1'),
        t('integrations.guide.tiktokPixel.req2'),
      ],
...
function GoogleCalendarModal({ activeTenant, onClose, onRefresh, t }) {
  ...
  return (
    <Modal title={t('integrations.modal.googleCalendar.title')} subtitle={t('integrations.modal.googleCalendar.subtitle')} onClose={onClose}>
      {loading ? (
        <div className="flex justify-center py-8"><Loader2 className="w-6 h-6 animate-spin text-[#8B1E3F]" /></div>
      ) : providers.length === 0 ? (
        <div className="text-center py-8">
          <AlertCircle className="w-12 h-12 text-gray-300 mx-auto mb-3" />
          <p className="text-sm text-gray-600 mb-1 font-medium">{t('integrations.modal.googleCalendar.noProviders')}</p>
          <p className="text-xs text-gray-500">{t('integrations.modal.googleCalendar.addProvidersHint')}</p>
        </div>
      ) : (
        <div className="space-y-3">
          <p className="text-sm text-gray-600 mb-4">{t('integrations.modal.googleCalendar.howItWorks')}</p>
          {providers.map((p) => (
            <div key={p.id} className="flex items-center justify-between p-4 rounded-xl bg-gray-50 border border-gray-200 hover:border-gray-300 transition-colors">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-gray-900">{p.name || t('integrations.modal.googleCalendar.providerFallback')}</p>
              ...
              {p.google_connected ? (
                <div className="flex items-center gap-2">
                  <span className="flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-green-100 text-green-700">
                    <Check className="w-3 h-3" /> {t('integrations.modal.googleCalendar.synced')}
                  </span>
                ...
                <button onClick={() => handleConnect(p.id)}
                  ...
                  <Link2 className="w-3.5 h-3.5" /> {t('integrations.modal.googleCalendar.connect')}
                </button>
              )}
            </div>
          ))}
          ...
          <div className="p-3 rounded-xl bg-blue-50 border border-blue-200 mt-4">
            <p className="text-xs text-blue-700">
              <strong>{t('integrations.modal.googleCalendar.autoSyncTitle')}</strong> {t('integrations.modal.googleCalendar.autoSyncDesc')}
            </p>
          </div>
        </div>
      )}
...
function WhatsAppModal({ activeTenant, onClose, onRefresh, t }) {
  ...
  return (
    <Modal title={t('integrations.modal.whatsapp.title')} subtitle={t('integrations.modal.whatsapp.subtitle')} onClose={onClose}>
      {phase === 'loading' && (
        <div className="flex flex-col items-center justify-center py-12">
          <Loader2 className="w-8 h-8 animate-spin text-green-600 mb-3" />
          <p className="text-sm text-gray-600">{t('integrations.modal.whatsapp.starting')}</p>
        </div>
      )}
      ...
      {phase === 'error' && (
        <div className="text-center py-8">
          ...
          <button onClick={startSession} className="px-4 py-2 rounded-xl bg-[#8B1E3F] text-white text-sm font-medium hover:opacity-90">{t('common.retry')}</button>
        </div>
      )}
      ...
      {phase === 'qr' && (
        <div className="text-center py-2">
          ...
          <p className="text-sm font-bold text-gray-900 mb-1">{t('integrations.modal.whatsapp.scanTitle')}</p>
          <p className="text-xs text-gray-500 mb-3">{t('integrations.modal.whatsapp.scanDesc')}</p>
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-green-50 border border-green-200">
            ...
            <span className="text-xs text-green-700 font-medium">{t('integrations.modal.whatsapp.waiting')}</span>
          </div>
        </div>
      )}
      ...
      {phase === 'authenticated' && (
        <div className="text-center py-6">
          ...
          <p className="text-lg font-bold text-gray-900 mb-1">{t('integrations.modal.whatsapp.connected')}</p>
          {phone && <p className="text-sm text-gray-600 mb-4 font-mono">{phone}</p>}
          <p className="text-xs text-gray-500 mb-6">{t('integrations.modal.whatsapp.connectedDesc')}</p>
          <button onClick={handleDisconnect}
            className="px-4 py-2 rounded-xl border border-red-200 text-red-600 text-sm font-medium hover:bg-red-50">
            {t('integrations.modal.whatsapp.disconnect')}
          </button>
        </div>
      )}
    </Modal>
...
function PixelConfigModal({ activeTenant, integration, onClose, onRefresh, t }) {
  ...
  return (
    <Modal title={integration.name} subtitle={t('integrations.modal.pixel.subtitle')} onClose={onClose}>
      <div className="space-y-5">
        ...
        {guide && (
          <div className="p-4 rounded-xl bg-gray-50 border border-gray-200">
            <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">{t('integrations.modal.pixel.whereToFind')}</p>
            ...
            {guide.docs && (
              <a href={guide.docs} target="_blank" rel="noopener noreferrer"
                ...
                <ExternalLink className="w-3 h-3" /> {t('integrations.modal.pixel.openDocs', { name: integration.name })}
              </a>
            )}
          </div>
        )}
        ...
        {guide?.tracked_events && (
          <div>
            <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">{t('integrations.modal.pixel.eventsWeTrack')}</p>
            <div className="flex flex-wrap gap-1.5">
              {guide.tracked_events.map((e, i) => (
        ...
        <div className="flex justify-end gap-3 pt-2">
          <button onClick={onClose}
            className="px-5 py-2.5 rounded-xl border border-gray-300 text-gray-700 font-medium hover:bg-gray-50 text-sm">{t('common.cancel')}</button>
          <button onClick={handleSave} disabled={saving || !allFilled}
            ...
            {t('integrations.modal.pixel.saveConnect')}
          </button>
        </div>
      </div>
    </Modal>
...
function DisconnectModal({ activeTenant, integration, onClose, onRefresh, t }) {
  ...
  return (
    <Modal title={t('integrations.modal.disconnect.title')} onClose={onClose}>
      <div className="text-center py-4">
        <div className="w-14 h-14 rounded-full bg-red-50 flex items-center justify-center mx-auto mb-4">
          <Unplug className="w-7 h-7 text-red-400" />
        </div>
        <p className="text-gray-900 font-bold mb-1">{t('integrations.modal.disconnect.confirm', { name: integration.name })}</p>
        <p className="text-sm text-gray-600 mb-6">{t('integrations.modal.disconnect.warning')}</p>
        <div className="flex justify-center gap-3">
          ...
          <button onClick={handleDisconnect} disabled={loading}
            ...
            {loading && <Loader2 className="w-4 h-4 animate-spin" />} {t('integrations.modal.disconnect.btn')}
          </button>
        </div>
      </div>
    </Modal>
