...
function ManageWhatsAppPageInner() {
  ...
  const isPlatformDisabled = session?.status === 'disabled_by_platform'
        ...
        <div className="bg-white rounded-2xl border border-gray-200">
          <div className="flex flex-col items-center justify-center py-20 px-8">
            ...
            <h2 className="text-2xl font-bold text-gray-900 mb-3">
              {t('whatsapp.disabled.title')}
            </h2>
            <p className="text-sm text-gray-500 text-center max-w-md">
              {t('whatsapp.disabled.desc')}
            </p>
          </div>
        </div>
      </div>
    ...
    <div className="space-y-6">
      ...
      {(hasError || (session?.phone && !isConnected)) && (
        <div className="flex items-start gap-3 p-4 rounded-xl bg-amber-50 border border-amber-300">
          ...
          <div className="flex-1">
            <p className="text-sm font-bold text-amber-900">{t('whatsapp.alert.title')}</p>
            <p className="text-sm text-amber-700 mt-0.5">
              {t('whatsapp.alert.desc')}{' '}
              <button onClick={() => setShowQRModal(true)} className="underline font-medium text-amber-800 hover:text-amber-900">
                {t('whatsapp.alert.here')}
              </button>{' '}
              {t('whatsapp.alert.continue')}
            </p>
            {session?.phone && (
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{t('whatsapp.breadcrumb')}</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-8">
          <div className="flex items-center gap-4 mb-6 pb-6 border-b border-gray-100">
            ...
            <div>
              <h2 className="text-lg font-bold text-gray-900">{t('whatsapp.connected.title')}</h2>
              <p className="text-sm text-gray-600">{t('whatsapp.connected.subtitle')}</p>
            </div>
            <div className="ml-auto flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-green-500 animate-pulse" />
              <span className="text-sm font-medium text-green-700">{t('whatsapp.connected.active')}</span>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div className="p-4 rounded-xl bg-gray-50 border border-gray-200">
              <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">{t('whatsapp.connected.phone')}</p>
              <p className="text-lg font-bold text-gray-900">{session.phone || t('common.unknown')}</p>
            </div>
            <div className="p-4 rounded-xl bg-gray-50 border border-gray-200">
              <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">{t('whatsapp.connected.statusLabel')}</p>
              <p className="text-lg font-bold text-green-700">{t('whatsapp.connected.statusValue')}</p>
            </div>
            <div className="p-4 rounded-xl bg-gray-50 border border-gray-200">
              <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">{t('whatsapp.connected.session')}</p>
          ...
          <div className="flex gap-3">
            <button onClick={() => setShowQRModal(true)}
              ...
              <RefreshCw className="w-4 h-4" /> {t('whatsapp.connected.relink')}
            </button>
            <button onClick={handleDisconnect}
              ...
              <Unplug className="w-4 h-4" /> {t('whatsapp.connected.disconnect')}
            </button>
          </div>
        </div>
      ) : (
        ...
        <div className="bg-white rounded-2xl border border-gray-200">
          <div className="flex flex-col items-center justify-center py-16 px-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-4">{t('whatsapp.disconnected.title')}</h2>
            ...
            <button onClick={() => setShowQRModal(true)}
              ...
              <QrCode className="w-5 h-5" /> {t('whatsapp.disconnected.link')}
            </button>
...
function QRLinkModal({ activeTenant, onClose, onConnected, t }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          <h2 className="text-lg font-bold text-gray-900">{t('whatsapp.modal.title')}</h2>
        ...
        <div className="px-6 py-6">
          {phase === 'loading' && (
            <div className="flex flex-col items-center py-12">
              ...
              <p className="text-sm text-gray-500">{t('whatsapp.modal.loading')}</p>
            </div>
          )}
          ...
          {phase === 'qr' && (
            <div className="flex flex-col items-center">
              ...
              <p className="text-sm font-medium text-gray-900 mb-4">
                {t('whatsapp.modal.scanTitle')}
              </p>

              {/* Instructions */}
              <div className="w-full text-left space-y-1.5 mb-5">
                <p className="text-sm text-gray-600">{t('whatsapp.modal.step1')}</p>
                <p className="text-sm text-gray-600">{t('whatsapp.modal.step2')}</p>
                <p className="text-sm text-gray-600">{t('whatsapp.modal.step3')}</p>
                <p className="text-sm text-gray-600">{t('whatsapp.modal.step4')}</p>
              </div>

              {/* Phone linking option */}
              <button className="text-sm text-[#8B1E3F] font-medium hover:text-[#6B1630]">
                {t('whatsapp.modal.linkWithPhone')}
              </button>
            </div>
          )}
        </div>
...
export default function ManageWhatsAppPage(props) {
  return (
    <PlanFeatureGate feature="whatsapp_integration">
