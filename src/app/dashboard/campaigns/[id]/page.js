    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Home → Campaigns → {campaign.name}</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <StatCard label="Status" value={campaign.status} color={
          { draft: 'text-gray-700', pending: 'text-amber-700', active: 'text-green-700', finished: 'text-blue-700', failed: 'text-red-700', suspended: 'text-purple-700' }[campaign.status]
        } />
        <StatCard label="Total Recipients" value={campaign.total_recipients.toLocaleString()} icon={Users} />
        <StatCard label="Sent" value={campaign.sent_count.toLocaleString()} sub={`${sentPercent}%`} />
        <StatCard label="Delivered" value={campaign.delivered_count.toLocaleString()} />
        <StatCard label="Failed" value={campaign.failed_count.toLocaleString()} color={campaign.failed_count > 0 ? 'text-red-600' : undefined} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-4">Campaign Details</h3>
          <dl className="space-y-3">
            <Row label="Audience" value={campaign.audience_display} />
            <Row label="Scheduled" value={campaign.scheduled_at ? new Date(campaign.scheduled_at).toLocaleString() : 'Immediate'} />
            <Row label="Started" value={campaign.started_at ? new Date(campaign.started_at).toLocaleString() : '-'} />
            <Row label="Finished" value={campaign.finished_at ? new Date(campaign.finished_at).toLocaleString() : '-'} />
            <Row label="Created by" value={campaign.created_by || '-'} />
            <Row label="Created" value={new Date(campaign.created_at).toLocaleString()} />
          </dl>
        </div>

        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-4">Message Content</h3>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50">
                  <th className="text-left px-6 py-3 text-xs font-bold text-gray-500 uppercase">Phone</th>
                  ...
                  <th className="text-left px-6 py-3 text-xs font-bold text-gray-500 uppercase">Sent At</th>
                  <th className="text-left px-6 py-3 text-xs font-bold text-gray-500 uppercase">Error</th>
                </tr>
              </thead>
...
function RecipientStatus({ status }) {
  const cfg = {
    ...
    sent:      { icon: Check, cls: 'text-blue-700 bg-blue-50' },
    delivered: { icon: CheckCircle2, cls: 'text-green-700 bg-green-50' },
    read:      { icon: CheckCircle2, cls: 'text-green-800 bg-green-100' },
    failed:    { icon: XCircle, cls: 'text-red-700 bg-red-50' },
  }[status] || { icon: Clock, cls: 'text-gray-500 bg-gray-50' }
