...
export default function Footer({ data, lang: propLang }) {
  ...
  const bgStyle = background === "primary" 
    ? { backgroundColor: theme.primary_color || "#1F2937" }
    : {};

  const mutedClass = background === "light" ? "text-gray-600" : "text-gray-400";
  ...
  return (
    <footer className={`${bgClass} ${isRTL ? "rtl" : ""}`} style={bgStyle}>
      <div className="max-w-7xl mx-auto px-6 py-16">
        {/* Main Footer Content */}
        <div className={`grid gap-12 ${resolvedColumns.length > 0 ? "md:grid-cols-4" : "md:grid-cols-2"}`}>
          {/* Brand Column */}
            ...
            {(contact_info.email || contact_info.phone || contact_info.address) && (
              <div className={`space-y-2 ${mutedClass}`}>
                ...
                {contact_info.address && (
                  <p className={`flex items-center gap-2 ${isRTL ? "flex-row-reverse" : ""}`}>
                    <span>📍</span> {resolveTranslated(contact_info.address, lang)}
                  </p>
                )}
              </div>
            )}
        ...
        {/* Bottom Bar */}
        <div className={`mt-12 pt-8 border-t ${background === "light" ? "border-gray-300" : "border-gray-700"} flex flex-col md:flex-row justify-between items-center gap-4`}>
          <p className={mutedClass}>
            {resolvedCopyright || `© ${new Date().getFullYear()} ${business_name || "All rights reserved"}`}
          </p>
